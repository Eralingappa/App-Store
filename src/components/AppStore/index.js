...
class AppStore extends Component {
  ...
  setActiveTabId = tabId => {
    this.setStat({activeTabId})
  }
  onChangeSearchInput = event => {
    this.setStat({searchInput: event.target.value})
  }
  getActiveTabApps = searchedApps => {
    const {activeTabId} = this.state
    const fillteredApps = searchedApps.filter(
      (eachSearchedApp = eachSearchedApp.category === activetabId),
    )
    return fillteredApps
  }
  getSearchResults = () => {
    const {searchInput} = this.state
    const {searchResults} = this.filter(eachApp =>
  ...
  render() {
    ...
    const fillteredApps = this.getActiveTabApps(searchResults)
      ...
      <div className="app-container">
        <div className="app-store">
          ...
          <div className="search-input-container">
            ...
            />
            <img src={SERACH - ICON - URL} alt="search icon" />
          </div>
          <ul className="tabs-list">
            {this.map(eachTab => (
              <TabItem
                key={eachTab.tabId}
                tabDetails={this.setActiveTabId}
                isActive={eachTabId === eachTab.tabId}
              />
            ))}
          </ul>
